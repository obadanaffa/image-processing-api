...
describe('API Endpoint Tests', () => {
  ...
  it('returns resized image for valid request', async () => {
    const response = await request.get('/api/images?filename=cse&width=200&height=200')
  ...
  it('returns 400 for invalid width or height', async () => {
    const response = await request.get('/api/images?filename=cse&width=-100&height=200')
    expect(response.status).toBe(400)
  })
